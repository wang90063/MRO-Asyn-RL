...
cell_x = [ 0.0*40, 170.0*40, 170.0*40, 0.0, -170*40, -170*40]
...
class SystemModel_wrong:
  ...
  def _move_user(self):
      ...
      index = np.random.choice(self.direction_index, p=[0.6, 0.2, 0.1, 0.1])
      self.users = self.users + self.direction[index] * mobility_speed

      if np.abs(self.users[0]) > np.sqrt(3)/2.0 * outer_radius or (np.abs(self.users[0])+np.abs(self.users[1])/np.sqrt(3)) > outer_radius: #and (np.abs(user_x_tmp) > np.sqrt(3)/2.0*inner_radius or (np.abs(user_x_tmp)+np.abs(user_y_tmp)/np.sqrt(3)) > inner_radius):
          self.terminal = True
...
def get_rate_percell(users, cells):
      ...
      for num in cell_id:
          ...
          norm_distance[num] = np.sqrt((cells[num][0] - users[0]) ** 2 + (cells[num][1] - users[1]) ** 2) * resolution# calculate the distance between user and each base station
      large_scale = 10**((-20 * np.log10(norm_distance+10**-4)-39+shadow_fading)/10)+10**-8
